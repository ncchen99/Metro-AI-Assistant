...
將 FastAPI 應用程式適配為 Vercel Serverless Functions
"""
...
請在 Vercel 專案設定中設定環境變數：
1. 前往 Vercel Dashboard
2. 選擇你的專案
3. 前往 Settings > Environment Variables
4. 新增：OPENAI_API_KEY=your_actual_api_key
5. 重新部署專案
...
async def root():
    ...
    return {
        ...
        "service": "Metro AI Assistant API (Vercel)"
    }
...
"""
                ...
                for chunk in stream:
                    if chunk.choices[0].delta.content is not None:
                        ...
                        # 發送回應片段
                        yield f"data: {json.dumps({'type': 'ai_response_chunk', 'chunk': content, 'timestamp': get_current_timestamp()})}\n\n"
                        await asyncio.sleep(0.01)  # 小延遲讓前端有時間處理
...
# 為 Vercel Serverless Functions 建立處理器
